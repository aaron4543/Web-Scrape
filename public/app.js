...
$(document).ready(function (event) {
    ...
    $("#scrape-articles").on("click", (event) => {
        ...
        function postArticles(image, header, summary, articleId) {
            ...
            let articleCards = $("<div>").attr("class", "article-cards col s12 card horizontal").attr("id", articleId); // **** NEED TO ADD ARTICLE ID ****
        ...
        let myArticleScraper = function (myScraped) {
            for (let i = 0; i < myScraped.length; i++) {
                const articleScraped = myScraped[i];
                postArticles(articleScraped.image, articleScraped.header, articleScraped.summary, articleScraped._id);
            }
        }
        // ============ inside functions
        $.getJSON("/articles", function (data) {
            myArticleScraper(data);
        });
